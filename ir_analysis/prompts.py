...
- 보고서는 역피라미드로 쓴다: 가장 중요한 '변화'와 관점 함의를 최상단 '핵심 변화(TL;DR)'
  블록에 요약하고(6~7개 bullet 이내 — 요약이 또 다른 벽이 되지 않게), 상세 수치·인용·경쟁표는
  그 뒤에, 데이터 신뢰도·기간 정합성·출처는 부록으로 맨 뒤에 둔다.
"""
...
def _output_format(target: str, persona_name: str, scope: str, competitive: bool,
    ...
    """scope별 보고서 골격 — 역피라미드(핵심 변화 → 관점 임플리케이션 → 상세 → 부록).

    직전 대비 톤 델타(delta_note)는 상세 톤 섹션이 아니라 최상단 '핵심 변화' 블록으로
    승격한다. 하우스키핑(데이터 신뢰도·기간 정합성·출처)은 부록으로 맨 뒤에 둔다.
    부록 안의 '### 출처' 헤딩은 유지한다 — analyze.find_previous_report 가 이 마커에서
    직전 보고서를 잘라 토큰을 아끼므로(출처 목록 제외) 절대 이름을 바꾸지 않는다.
    """
    delta_note = ("\n- **직전 대비 톤 델타(1~2줄)**: 새로 등장/사라진 메시지 · "
                  "확신·경계 수위가 바뀐 지점 · 가이던스 상향·하향 (직전 보고서 제공됨)"
                  if has_history else "")
    comp_on = competitive
...
## 핵심 변화 (TL;DR)
- **이번 분기 한 줄**: 무엇이 바뀌었나 — 방향과 폭을 한 문장으로 (기존 '한 줄 결론'을 여기로 흡수)
- **핵심 변화 3~4개**: 각 항목을 [직전·예상 대비 변화 + 숫자 + 방향 + 함의] 한 줄로{delta_note}
- **{persona_name} 관점 함의(1줄)**: 아래 임플리케이션의 결론만 미리
- **주가 반응 + 이유(1줄)**
- ⚠️ **착시·주의 1개**: 겉보기와 다른 지점(일회성·시점차·회계 등)
(요약이 또 다른 벽이 되지 않게 — 전체 6~7 bullet 이내)

## {persona_name}를 위한 임플리케이션
...
## 상세 — 가이던스 품질
(범위·가정·전분기 가이던스 대비 변경점, 달성 가능성에 대한 근거){comp_sec}
...
## 핵심 변화 (TL;DR)
- **이번 콜 한 줄**: 톤·메시지가 어떻게 바뀌었나를 한 문장으로 (기존 '한 줄 결론'을 여기로 흡수)
- **핵심 변화 3~4개**: 각 항목을 [직전·예상 대비 톤/메시지 변화 + 발췌 앵커 + 방향 + 함의] 한 줄로{delta_note}
- **{persona_name} 관점 함의(1줄)**
- **발표 직후 시장 반응 + 이유(1줄)**
- ⚠️ **주의 1개**: 말과 숫자의 괴리 등 겉보기와 다른 지점
(요약이 또 다른 벽이 되지 않게 — 전체 6~7 bullet 이내)

## {persona_name}를 위한 임플리케이션

## 상세 — 콜 개요
(참석 경영진, prepared remarks/Q&A 구조)

## 상세 — Prepared Remarks 주제별 발췌 딥다이브
...
## 상세 — 말과 숫자의 괴리
(발언 대비 공시 수치의 긴장 지점 — 숫자는 대조에 필요한 최소만){comp_sec}
...
## 상세 — 경쟁 현황 정량 비교 (경쟁 분석 ON)
(세 회사를 같은 지표로 나란히 + 마지막 '핵심 차이(1줄)' 열, 일관 톤, 시점차 명시)
...
## {persona_name}를 위한 임플리케이션
(렌즈별로 구조화: 방어 / 공세 / 구조 점검 / 톤 벤치마크 등 — 숫자 근거를 반드시 붙인다)

## 상세 — {target} 정량 요약
(핵심 수치 표 + 가이던스, 컨센서스 대비·일회성 항목 별표 처리)

## 상세 — {target} 컨콜 정성(톤) 발췌 분석
(항목별로 ① 발언 원문(영문) 발췌 → ② 해석 → ③ 관점 시사점. 자신감/공급관/점유율/가격/로드맵 등){comp_sec}
